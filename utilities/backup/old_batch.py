...
# - list of indices of epidemics
# - sizes of epidemics
# - durations of epidemics
def breakepis(x, sensitivity, maxdur = None) :
...
for idx, file in enumerate(directory) :
	...
	epi, reals, reald, maxdurs = breakepis(C, sensitivity)
	epi = np.array(epi)
	...
	for q in range(int(sys.argv[2])) :
	    ...
	    for index, e in enumerate(starts) :
		    ...
		    _, simsize, simdur, _ = breakepis(predI, sensitivity, maxdurs)
